# Eudaimonia as Alignment Target

## Why Eudaimonia?

Most alignment frameworks define "aligned" negatively: an AI that doesn't cause harm, doesn't deceive, doesn't act outside sanctioned boundaries. These constraints are necessary but insufficient. A system that merely *avoids bad* is not the same as a system that *does good*.

Aristotle's concept of eudaimonia — often translated as "flourishing" or "living well" — offers something different: a positive account of what excellence looks like for an intelligent agent.

## The Aristotelian Framework

For Aristotle, eudaimonia is not a feeling (hedonia) but an activity: *energeia kat' areten* — activity in accordance with virtue. Three concepts are central:
...
| Aristotelian Concept | Alignment Implication | Computational Form |
|---------------------|----------------------|--------------------|
| Arete | Values as character, not constraint | Constitutional identity layer |
| Phronesis | Contextual judgment under uncertainty | Bayesian governance with prior on flourishing |
| Telos | Positive goal structure | Eudaimonic reward specification |
| Eudaimonia | Flourishing as objective | Free energy over eudaimonic prior |
